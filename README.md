...
- 本機可用的文字日記新增流程
- Apple Journal ZIP 檔名、大小與 ZIP signature 基本檢查
...
目前 remote 寫入預設關閉。只有 `localhost` 可以建立 entry；正式環境必須先完成 Cloudflare Access JWT 驗證，才可開放任何 mutation。Apple Journal 完整解析仍需真實但去識別化的 export fixture 驗證內部格式，UI 不會把基本 ZIP 檢查冒充為完成匯入。
...
因此匯入器不能只根據網路上的單一範例硬寫。開始實作前，需要建立一份不含真實隱私的最小 Apple Journal fixture，至少包含：
...
- ZIP 由瀏覽器使用 streaming/random-access 方式逐項讀取，不將整包送進 Worker 或一次解壓到記憶體。
- 媒體使用短效 presigned URL 直接上傳 R2，避開 Workers Free/Pro 100 MB request body 限制。
- 大型單檔使用 R2 multipart upload；每個 part 可重試。
- import job 會記錄進度；關閉頁面後重新選擇同一個 ZIP，可根據檔案 fingerprint 與 checksum 續傳。
- 原始 ZIP 預設不永久保存，避免同一份媒體占兩倍空間；可在設定中選擇「保留原始匯入備份」。
- 每一篇 entry 獨立 commit。某個附件失敗不能讓已完成的數百篇全部 rollback。

### 冪等性與安全

- 優先使用 Apple 匯出提供的穩定 ID；若沒有，使用 canonical entry fields 的 SHA-256 作為 `source_hash`。
- `(source, source_id)` 或 `(source, source_hash)` 建 unique constraint，重跑匯入不產生重複日記。
- 媒體以 SHA-256 去重；相同檔案可被多篇 entry 引用。
...
- 取得合成 Apple Journal export fixture 並記錄 schema 變異
