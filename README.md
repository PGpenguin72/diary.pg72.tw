...
- 已實作：part、complete 與 abort 都攜帶 entry generation，並在 D1 reservation / commit 同時比對 upload ID、upload row generation/version/state 與 entry 的 current generation；每個 R2 await 後的 commit、failure release 與 terminal transition 都重新檢查受影響筆數，期間切換 generation 固定回 409，不能改動 replacement upload 或寫入成功報告。各動作再以 version/next-part compare-and-set 互斥；part reservation 有短 lease，Worker 在 R2 寫入前後中止時可由同一 part 安全接管並重寫。R2 已完成但 D1 尚未 finalize 時，以 private object head 重試 reconciliation；錯誤大小的無引用 object 只在 exact failed transition 後排入 durable cleanup，不同步刪除可能已由新 generation 接管的 key。
