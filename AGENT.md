...
Remote mutations are intentionally denied until complete Cloudflare Access JWT verification is implemented. Do not weaken this to a client-provided email, a spoofable header, or a hostname-only production allow rule.
...
- Protect every app and API route with the chosen authentication boundary. For the single-user MVP, this is Cloudflare Access with an exact email allowlist.
- Validate the Access assertion server-side. Never trust a client-provided email or a hidden UI state as authorization.
