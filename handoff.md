...
> 實作基線：`8073d88 fix: serialize diary media imports`；本文件所在 commit 請以 `git log -1` 為準
...
- 每日 scheduled cleanup 分批回收過期 session，保護 active lease、先修復 R2-success / D1-pending，再 abort；terminal bookkeeping 保留 7 天後刪除，ready media 與 R2 object 不受影響。
- import job 與 import item 會記錄進度；UI 顯示整體 item 與目前檔案 byte/part 進度，部分失敗可逐項檢視、下載 JSON report，並允許重跑同一 ZIP。
- parser 對單次 preview 設 10,000 entries、32 MiB HTML 與 32 MiB retained text 上限；損壞 ZIP 和不安全路徑有 desktop/mobile regression。
...
2. **大型媒體 upload 的 production gate**：本機已改為 8 MiB Worker request + private R2 multipart，含順序/大小/MIME/簽章/ETag 驗證、逐 part retry、續傳、取消、CAS crash recovery、排程 cleanup 與 failed-row 修復；仍需重新審核本 branch、先套用 `0003_media_uploads.sql`，再用合成大檔做 production smoke。presigned direct upload 只是可選流量優化。
3. **匯入 reconciliation**：已有整體與逐檔進度、去重、逐 part 續傳、R2/D1 reconciliation、可下載 error report 與部分失敗重試 UI；end-to-end content checksum reconciliation 仍需補強。
...
目前 production 明確為 **NO-GO**：先完成 branch re-review、production/preview resource isolation、migration `0003` 與無個資合成 smoke，才可部署或匯入真實資料。
...
2. 重新審核 multipart/CAS/cleanup 變更：先在隔離環境套用 migration `0003`，再用合成媒體測 retry/resume/abort、scheduled cleanup 與大於 100 MB 單檔；不要先用真實日記試錯。
...
- `test/worker.test.ts` / `test/import-media-upload.test.ts`：migration、API、匯入、D1/R2、dedupe、CAS crash recovery、cleanup 與虛擬大檔 geometry；目前完整 Vitest 為 62 tests。
- `e2e/diary.spec.ts`：desktop/mobile overview、malformed/valid-preview/partial-failure import、逐檔進度、composer、Apple import/re-import、Markdown、自然比例 media masonry、lightbox、卡片無媒體，以及有序 timeline masonry；目前為 8 Playwright cases。
